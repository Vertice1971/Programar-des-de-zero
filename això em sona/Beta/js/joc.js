...
// -------------------------------------------
// Aquest script implementa la lògica principal del joc "Això em sona"
// complint les especificacions: mode individual i per torns,
// opcions múltiples, temporitzador, imatges de fons i control de rondes.
...
function demanarConfiguracio() {
  ...
  let rondes;
  while (true) {
    const entrada = prompt(`Quantes rondes vols jugar? (1 ‑ ${maxRondes})`, '1');
...
function continuarDesprésResultat() {
  timeoutResultatId = setTimeout(() => {
    const quedenRondes = rondaActual < numRondes || (jugadorActualIndex + 1) % jugadors.length !== 0;
